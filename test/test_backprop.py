...
class TestBackprop:
    ...
    def test_backprop1(self):
        ...
        assert torch.allclose(t8, t7, atol=1e-3, rtol=0)
        t9 = torch.frombuffer(arr3.grad, dtype=torch.float32)
        assert torch.allclose(t9, t3.grad, atol=1e-3, rtol=0)
        t10 = torch.frombuffer(arr4.grad, dtype=torch.float32)
        assert torch.allclose(t10, t4.grad, atol=1e-3, rtol=0)
        t11 = torch.frombuffer(arr5.grad, dtype=torch.float32)
        assert torch.allclose(t11, t5.grad, atol=1e-3, rtol=0)
        t12 = torch.frombuffer(arr6.grad, dtype=torch.float32)
        assert torch.allclose(t12, t6.grad, atol=1e-3, rtol=0)
        t13 = torch.frombuffer(arr7.grad, dtype=torch.float32)
        assert torch.allclose(t13, t7.grad, atol=1e-3, rtol=0)
