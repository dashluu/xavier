...
namespace xv::backend::metal
{
    ...
    void MTLContext::init_kernels(const std::string &op, const std::unordered_set<Dtype> &dtypes, const std::string &mode)
    {
        ...
        {
            auto name = (mode.empty() ? "" : mode + "_") + op + "_" + dtype.get_name();
    ...
    void MTLContext::init_initializer_kernels()
    {
        ...
        init_kernels("arange", num_dtypes);
    }

    void MTLContext::init_unary_kernels()
    {
        init_kernels(num_unary_ops, num_dtypes);
        init_kernels(num_unary_ops, num_dtypes, "sparse");
    }

    void MTLContext::init_binary_kernels()
    {
        init_kernels(num_binary_ops, num_dtypes);
        init_kernels(num_binary_ops, num_dtypes, "sparse");
        init_kernels("matmul", num_dtypes);
        init_kernels("matmul", num_dtypes, "sparse");
    }

    void MTLContext::init_util_kernels()
    {
        init_kernels("copy", all_dtypes);
        init_kernels("copy", all_dtypes, "sparse");
    }
